...
- ну можно взять как раз разницу безработицы и впихнуть в лучшую модель  

```{r autoplot unempl diff}
ggplot(df3_ts,aes(x=Date,y=difference(Value*100)))+
  geom_line(color='grey30',lty=5,size=0.4)+
  geom_line(data=df2_ts,aes(x=Date,y=Value/1e+6),color='#0066cc',size=0.4)+
  theme_cowplot()+
  background_grid(size.major = 0.2)+
  scale_x_yearmonth(date_breaks = "6 month", date_labels = "%y'%m")+
  labs(title = "Кол-во ночей в гостинице", subtitle = "Безработица прирост (х100)",
