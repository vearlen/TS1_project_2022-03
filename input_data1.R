...
df2 = read.csv("tour_occ_nim_1_Data.csv")
View(df2)
...
df3 = read.csv("ei_lmhr_m_1_Data.csv")
...
gg_tsdisplay(df2_ts)
head(df2_ts)
nights = df2_ts %>% filter(Date < ymd('2020-02-01'))
stl_model = model(df2_ts,
                  decomp = STL(Value ~ trend(window = 100) +
                                 season(window = 5)))
stl_model = model(nights,
                  decomp = STL(Value ~ trend(window = 50) +
                                 season(window = 5)))
components(stl_model) %>% autoplot()

#df2_ts = mutate(df2_ts, diff = difference(Value, order_by = Date))
#gg_tsdisplay(df2_ts, y = diff)
#df2_ts = mutate(df2_ts, diff_seas = difference(Value, lag = 12, order_by = Date))
#gg_tsdisplay(df2_ts, y = diff_seas)

nights = mutate(nights, diff = difference(Value, order_by = Date))
gg_tsdisplay(nights, y = diff, plot_type = 'partial')
nights = mutate(nights, diff_seas = difference(Value, lag = 12, order_by = Date))
